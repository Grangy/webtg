...
export async function GET(
  ...
  try {
    ...
    // Баланс: берём из GET /user/:id/balance. Поле balance — уже общий (включая реферальные начисления).
    // balanceReferral/referralBalance — только для отображения "в т.ч. с рефералов", НЕ суммируем.
    let totalBalance = 0;
